...
def main(expt_name, config):
  ...
  df = read_csv('results.csv', config)
  if df is None:
    ...
    for (campaign_bg_event, campaign_id), predictions_campaign_df in predictions_df.groupby([FeatureName.CAMPAIGN_BG_EVENT, FeatureName.CAMPAIGN_ID]):
      ...
      for forecast_date, sub_df in predictions_campaign_df.groupby('forecast_date'):
        ...
        known_days = (sub_df['horizon'] == -1).sum()
        sub_df['y_pred_hybrid'] = np.where(
          sub_df['horizon'] < 0,
          sub_df['y_true'],
          (sub_df['y_pred_benchmark'] * known_days + sub_df['y_pred'] * sub_df['horizon']) / (known_days + sub_df['horizon']),
        )
        ...
        dfs.append(sub_df[[
          ...
          'y_true', 'y_pred', 'y_pred_benchmark', 'y_pred_hybrid', 'history',
        ]])
  ...
  for (horizon, history), sub_df in df.groupby(['horizon', 'history']):
    ...
    rmsle_hybrid = int((np.exp(rmsle(sub_df['y_true'], sub_df['y_pred_hybrid'])) - 1) * 100)
    errs.append({
      'horizon': horizon,
      'history': history,
      'value': f'{rmsle_model}% / {rmsle_hybrid}% / {rmsle_benchmark}% / {len(sub_df)}',
    })
